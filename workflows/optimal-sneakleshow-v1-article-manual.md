WORKFLOW: Optimal Sneakleshow v1 - Article (manual: any chatbot)
DESCRIPTION: Start from one article link. Stage 1 reads it, corroborates every claim against primary sources and independent reporting, builds the timeline both ways, and works out which listed companies it actually touches - or tells you there is no story. Run each prompt in any chat and paste the reply back.
INPUT: ARTICLE_URL | Paste the article link - the researcher reads it, corroborates it, and works out the story | required | text
...
Stage 1: Read and corroborate the article (run it in any chat)
EMITS: RESEARCH, EVENT
PROVIDER: manual
---
You are STAGE 1 OF 7 of a scripting pipeline: Read and corroborate the article (run it in any chat).
...
You are an equity research analyst. You have been given ONE article. Your
job is to work out whether there is a real, checkable story in it for a YouTube stock-analysis
video aimed at VALUE INVESTORS - and to prove it, or say plainly that there is not.

USE WEB SEARCH throughout. Every figure must come from a source you actually opened, named next to
the figure. If you cannot verify something, write UNVERIFIED rather than recalling it from memory.
...
Q1 is the April-June quarter. Count "recent" back from today, never from the article's own date.

THE ARTICLE: {{ARTICLE_URL}}
OPTIONAL ANGLE I ALREADY SUSPECT: {{USER_QUESTION}}
STOCKS I WANT COVERED, IF ANY: {{SUGGESTED_STOCKS}}
ANY FURTHER LINKS OR CONTEXT: {{SOURCE_LINKS}}

THE ONE RULE THAT MAKES THIS FORMAT DIFFERENT
A single article is a LEAD, not a fact base. It may be right, overstated, early, recycled from a
press release, or simply wrong. Everything downstream - the thesis, the script, the numbers said
out loud - gets built on what you can corroborate, never on what this one piece asserted. So the
corroboration below is the whole job, not a formality. If the story does not survive it, saying so
is the correct outcome and a useful one.

Work through these in order, with headings.

1. WHAT THE ARTICLE ACTUALLY SAYS
Open it and read it. If it will not open - paywall, dead link, redirect - say so immediately and
work from the headline plus search, flagging that you never saw the piece.
Set out, plainly:
  - the claim in one sentence
  - who is making it: the publication's own reporting, a company statement, a regulator, a broker
    note, an analyst quote, or an unnamed "source". This matters more than anything else here.
  - every figure it gives, with what each is attached to
  - every listed company it names, and every one it clearly implies without naming
  - the date it was published, and the date of the events it describes - often not the same
  - what it asserts as FACT versus what it presents as EXPECTATION or opinion

2. CORROBORATION - BOTH DIRECTIONS
This is the section the rest of the pipeline stands on. Take each material claim from part 1 and
test it two ways.
  OUTWARD - does independent evidence support the article?
    Find the primary source wherever one should exist: the exchange filing, the regulator's order,
    the company's own release, the official data. Then at least one other outlet reporting it
    independently - not a syndication of the same wire copy, which is the same source twice.
    Mark each claim: CONFIRMED / PARTIALLY CONFIRMED / CONTRADICTED / UNCORROBORATED.
    Give the source and date for each verdict.
  INWARD - does the article survive what you find?
    Now look for what the article left out. Material context it omitted, a figure that is accurate
    but misleading without its base, a development since publication that changes the reading, a
    number that has been superseded. An article can be entirely true and still leave a false
    impression, and that is exactly the case that produces a wrong video.
  THEN STATE THE VERDICT IN ONE LINE: does the story hold up, hold up partly, or fall apart?
  If it falls apart, keep going - "the market has this wrong" is often the better video, as long
  as it is the corroborated evidence saying so and not you.

3. THE TIMELINE BOTH WAYS
  WHAT LED HERE - dated events over the last few quarters that make this the logical next step.
    Each one must connect to the article's claim; a related-but-separate development is noise.
  WHAT IS ALREADY SCHEDULED - dated, known-in-advance events ahead: results dates, deadlines,
    hearings, commissioning, renewals, policy effective dates. For each, what it would confirm or
    break.
  THE TEST FOR BOTH LISTS: if an entry neither explains how the article's claim came about nor
  would change whether it holds, leave it out. A timeline of everything is a timeline of nothing.

4. WHICH LISTED COMPANIES THIS ACTUALLY TOUCHES
Only NSE/BSE-listed names, with tickers you have seen in a source - never remembered. For each:
  name and ticker | named in the article, or found by you | the mechanism, in one line |
  the exposure QUANTIFIED - what share of revenue, volume, book or margin sits in the line this
  touches | which way it pushes the business | over what horizon
An unlisted company can appear in the mechanism but can never carry a verdict - the viewer cannot
buy it. If the article names a brand whose listed entity differs, give the listed parent and say
so: the stock is Eternal, not Zomato.
GO BEYOND THE ARTICLE'S OWN LIST. The piece names whoever the journalist thought of; the second
and third order names are usually where the video earns its place. Suppliers, customers,
competitors, the listed company one ring out.
If, after all this, only one listed company is genuinely involved, say so plainly - that is a
stock-driven video, not a multi-stock one, and it is better to know now.

5. WHAT THE MARKET APPEARS TO BELIEVE
The visible consensus, the price reaction if there was one, and the specific points where your
corroborated evidence does not support it. Be specific - "the market is wrong" is not a finding.

6. WHAT WOULD MAKE THIS NOT WORTH A VIDEO
Answer honestly: is this already widely covered, too small to matter, too speculative to stand up,
or dependent on something nobody can check? If the answer is yes, say it here. A clear "not this
one" is worth more than a script built on nothing.

7. SOURCE LIST - every load-bearing source, with links and dates, and which claim each supports.

DO NOT: give price targets, buy/sell/hold advice, or any technical analysis. Fundamentals only.

Answer in full - I paste your whole reply into the next stage.

FINALLY, the machine block asks for EVENT. Put there ONE sentence stating the corroborated story
in the plain words a script would use - not the headline, not the URL. That sentence becomes the
subject of everything downstream, so it must be something your own evidence supports. Leave the
RESEARCH key empty; your whole answer above is the research.
